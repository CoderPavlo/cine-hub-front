...
export default function MovieBlock({title, page, setPage, data, isFetching, error, refetch, genresData, top}:MovieBlockProps) {
    useEffect(()=>{
          if (data) {
            ...
            <Grid2 container spacing={3}>
              {data && data.results.map((movie) => (
                <Grid2 size={{xs: 12, sm: 6, md: 4}} key={movie.id}>
                  <MovieCard movie={movie} genresData={genresData}/>
                </Grid2>
              ))}
            </Grid2>
        </LoadingComponent>
